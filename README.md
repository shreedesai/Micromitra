...
Go to https://supabase.com → sign up (free tier is fine) → New project.
Pick a name (e.g. `micromitra`), a database password (save it somewhere safe), and a region close to your students.
Once the project finishes provisioning, go to SQL Editor (left sidebar) → New query.
Open `supabase/schema.sql` from this project, paste its entire contents in, and click Run.
...
Go to Project Settings → API. Copy two values, you'll need them shortly:
...
Go to API Keys → Create Key. Copy it — you'll paste it into Render, never into the HTML file.
Add billing (Anthropic's API is pay-as-you-go; it's separate from any Claude.ai subscription).
4. Put this project on GitHub
Render deploys from a GitHub repo.
Create a new repository on https://github.com (e.g. `micromitra`).
Upload all the files in this project folder (`server.js`, `package.json`, `public/`, `supabase/`) to that repo — either via GitHub's web "upload files" button, or `git push` if you're comfortable with git.
5. Deploy on Render
Go to https://render.com → sign up (free tier works) → New + → Web Service.
Connect your GitHub account and select the repo you just created.
Configure:
Runtime: Node
Build Command: `npm install`
Start Command: `npm start`
Instance type: Free is fine to start
Under Environment, add one environment variable:
Key: `ANTHROPIC_API_KEY`
Value: (paste the key from step 3)
Click Create Web Service. Render will build and deploy — takes a couple of minutes.
Once live, Render gives you a URL like `https://micromitra.onrender.com` — that's your app, running independently of Claude.ai.
...
Sign up as Faculty with your own email — this becomes your admin account.
Have a student (or yourself, in a private/incognito window) sign up as Student.
Use the AI Tutor as the student a few times, then log in as faculty and check Class Dashboard / Admin Panel — you should see the real student's data appear (it may take a page refresh).
---
